...
import { garageApi, stripeApi } from '../api/index.js';
import { CreditCard, CheckCircle, Zap, Building2, ArrowUpRight } from 'lucide-react';
import { toast } from 'sonner';

const plans = {
  starter: { name: 'Starter', price: 69, limit: 15, icon: Zap },
  business: { name: 'Business', price: 199, limit: null, icon: Building2 },
};
...
export default function Billing() {
  ...
  const handleSubscribe = async (planId) => {
    try {
      const response = await stripeApi.createCheckoutSession(planId, garage.id);
      ...
      <div className="bg-white/[0.03] rounded-xl p-6 border border-white/[0.06]">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex gap-2">
            ...
            <Link
              to="/pricing"
              className="flex items-center gap-2 px-4 py-2 bg-[#007AFF] text-white rounded-lg font-medium hover:bg-[#007AFF]/90 transition-colors"
            >
              {garage?.is_subscribed ? 'Changer' : 'Voir les plans'}
            </Link>
          </div>
        </div>
      </div>

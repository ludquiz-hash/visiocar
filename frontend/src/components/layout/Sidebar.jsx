...
const menuItems = [
  ...
  { id: 'new-claim', label: 'Nouveau dossier', icon: Plus, path: '/wizard', highlight: true },
  { id: 'team', label: 'Équipe', icon: Users, path: '/team' },
  { id: 'billing', label: 'Abonnement', icon: CreditCard, path: '/billing' },
  { id: 'garage', label: 'Mon garage', icon: Building2, path: '/settings' },
...
export default function Sidebar() {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-screen w-64 z-40 bg-[#0B0E14]/95 backdrop-blur-xl border-r border-white/[0.06]">
        {/* Logo */}
        ...
        <div className="p-3 border-t border-white/[0.06]">
          ...
          <button
            onClick={signOut}
            className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/[0.06] transition-colors mt-1"
          >
      ...
      {/* Mobile Bottom Nav */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#0B0E14]/95 backdrop-blur-xl border-t border-white/[0.06] px-2 py-2">
